...
def testOnImg(CNN, im, gtdots=None, pw=None, mask = None):
    ...
    # plt.show() 
    # kernel = np.ones((1,1),dtype='float')*(1.0/(1*1))
    # resImg = cv2.filter2D(resImg,-1,kernel)
    # output_show = cv2.blur(resImg,(3,3))
    output_show = 255*resImg/np.max(resImg)
    # plt.imshow(im)
...
def main(argv):
    ...
    caffemodel_path='/media/tmx/Gin/counting/datasets/new_add/ours_scale/ccnn_trancos_iter_45000.caffemodel'
